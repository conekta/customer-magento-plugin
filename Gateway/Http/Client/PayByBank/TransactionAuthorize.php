...
namespace Conekta\Payments\Gateway\Http\Client\PayByBank;
...
class TransactionAuthorize implements ClientInterface
{
    ...
    )
    {
        ...
        $this->_conektaLogger->info('HTTP Client PayByBank TransactionAuthorize :: __construct');
        $this->logger = $logger;
    ...
    {
        $this->_conektaLogger->info('HTTP Client PayByBank TransactionAuthorize :: placeRequest');
        ...
        try {
            ...
            $this->_conektaLogger->info(
                'HTTP Client PayByBank TransactionAuthorize :: placeRequest: Payment authorize error ' . $e->getMessage()
            );
        ...
        // Store Pay By Bank specific data
        $paymentMethod = $charge->getPaymentMethod();
        $response['offline_info'] = [
            "type" => $paymentMethod->getType(),
            "data" => [
                "reference" => $paymentMethod->getReference() ?? '',
                "expires_at" => $paymentMethod->getExpiresAt() ?? '',
                "redirect_url" => $paymentMethod->getRedirectUrl() ?? '',
                "deep_link" => $paymentMethod->getDeepLink() ?? ''
            ]
        ];
        ...
        $this->_conektaLogger->info(
            'HTTP Client PayByBank TransactionAuthorize :: placeRequest',
            [
                'request' => $request,
                'response' => $response
            ]
    ...
    {
        $this->_conektaLogger->info('HTTP Client PayByBank TransactionAuthorize :: generateResponseForCode');
